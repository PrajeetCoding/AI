...
def minimax(side, board, flags, depth):
    ...
    if depth == 0:
        return (chess.lib.heuristics.evaluate(board), moveList, moveTree)
    elif not side:
        value = float('-inf')


        for move in generateMoves(side,board,flags):
            m = encode(move[0], move[1], move[2])
            newside, newboard, newflags = chess.lib.makeMove(side, board, move[0], move[1], flags, move[2])

            tmp = value
            recurse = minimax(newside, newboard, newflags, depth - 1)
            value = max(value, recurse[0])
...
def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if depth == 0:
        ...
        for move in generateMoves(side,board,flags):
            ...
            recurse = alphabeta(newside, newboard, newflags, depth - 1, alpha, beta)
            value = max(value, recurse[0])
